...
【硬规则】
1. 每个板块你要同时产出两样东西：a) 图像生成提示词（prompt 字段）与 b) 营销文案包（copy 字段，含 title 主标题≤10字、subtitle 副标题≤20字、points 卖点短句数组≤3条每条≤15字）。prompt 必须以「用途」起句，明确这是用于[目标平台]电商的[板块类型]商品图（如"淘宝电商主图""抖音电商详情图"），并在五要素（画面主体、场景、光线、构图、风格）基础上描述该平台电商图典型特征：淘宝/京东——白底或浅纯色背景、产品居中突出、细节清晰、信息层级分明；抖音——真实使用场景与氛围感、生活化构图、适合竖屏；小红书——ins风高级感、生活方式化；亚马逊——纯白背景（强制）、产品占画面85%以上、无文字无logo、大图。
    ...
    messages: [
      {
        ...
        content: [
          {
            type: 'text',
            text: '请从电商详情图策划角度分析这张商品图，输出四部分：1)品类；2)外观/材质/颜色/工艺特征；3)结构特征；4)可提炼的电商视觉卖点（4-6条短句，从图中视觉信息直接看出，如高端包装、品牌辨识度、色泽饱满、便携设计等）。简洁列出，不要解释。',
          },
